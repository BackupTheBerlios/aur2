...
urlpatterns = patterns('aur.views',
    ...
    (r'^api/search/(?P<query>[\w\d_ -]+).(?P<format>(json|xml))$',
        'api_search'),
    (r'^api/package/(?P<object_id>[\w_-]+).(?P<format>(json|xml))$',
        'api_package_info'),
    (r'^api/package/(?P<object_id>[\w_-]+)/comments.(?P<format>(json|xml))$',
        'api_package_comments'),
